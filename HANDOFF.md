...
- This checkpoint passes 92 web unit-test files and 749 tests, the Worker,
  ...
  owner was away.
- Native Show Board is now a complete primary surface. Its dedicated
  `showboard:view` endpoint returns only the active organization crew, venue
  timezone, and 12/24-hour clock preference. The responsive phone/tablet board
  shows the live venue clock, checked-in/offline totals, crew photos or initials,
  roles, and three-second status convergence; pull-to-refresh and an explicit
  refresh control remain available. The Operations menu exposes the board only
  to authorized roles. Crew can open or share the canonical public check-in URL.
  Its QR is a cached, first-party SVG generated by the existing web QR engine,
  so no workspace data reaches an external QR service.
- A first native QR dependency worked but increased every Hermes bundle by
  roughly 670 KB. It was removed before commit. Reusing the first-party SVG
  endpoint adds only about 23 KB over the preceding native checkpoint. Rendered
  Expo web QA at 320x700 caught and fixed a wrapped ShowPilot wordmark and
  floating controls that obscured crew cards. The final 320x700 and 768x1024
  boards have no clipping, overlay, console error, or page error; the refresh
  action issued a fresh scoped read. The Show Board checkpoint passes 93 web
  unit-test files and 752 tests, the Worker and control suites, web/native
  TypeScript, native lint, the production web build, Expo Doctor 18/18, and real
  iOS/Android/web exports. Hermes bundles are 5,265,652 bytes on iOS and

...
const avatarUploadSchema = z.object({
  url: z
    .string()
    .refine(
      (value) => resolveMobileAvatarUrl(value) !== null,
      "Invalid avatar URL",
    ),
});
...
const chatNotificationResultSchema = z.object({
...
export type MobileRundownTemplate = z.infer<
  typeof mobileRundownTemplatesSchema
>["templates"][number];
export type MobilePreviousRundown = z.infer<
  typeof mobileRundownTemplatesSchema
...
const incidentReactionEmojiSchema = z
  .string()
  .refine(isEmojiReaction, "Choose an emoji");
...
const incidentsSchema = z.object({
  ...
  responders: z
    .array(z.object({ userId: z.string(), name: z.string(), role: z.string() }))
    .default([]),
...
const inviteTeamMemberResponseSchema = z.object({
  invitation: organizationInvitationSchema,
});

export const checklistDepartmentSchema = z.enum([
  "audio",
  "video",
  "lighting",
  "stream",
...
const mobileChecklistShowsSchema = z.object({
  shows: z.array(checklistShowSchema),
});
...
const checklistApplySchema = okSchema.extend({
  ...
  devices: z.array(
    z.object({
      ...
      configuration: z.array(
        mobileDeviceFieldSchema.extend({
          value: z.string(),
...
const assetSchema = z.object({
  ...
  notes: z.string(),
  lastServiced: z.string().nullable(),
  nextService: z.string().nullable(),
  revision: z.number().int().min(0),
  updatedAt: z.string(),
});
...
const reportsSchema = z.object({
  ...
  viewer: z.object({
    userId: z.string(),
    writableNoteLanes: z.array(z.enum(["pm", "tm"])),
  }),
});
...
const audioAssignmentSchema = z.object({
  ...
  muted: z.boolean(),
  group: z.string(),
  mixerConsole: z.string(),
  mixerChannel: z.number().nullable(),
  mixerChannelType: z.string(),
  serviceDate: z.string(),
  updatedAt: z.string(),
});
const audioSchema = z.object({
  ...
  shows: z.array(mobileOperationalShowSchema),
  mixers: z.array(
    z.object({ id: z.string(), name: z.string(), adapterType: z.string() }),
  ),
  assignments: z.array(audioAssignmentSchema),
});

const timecodeValueSchema = z.object({
  hours: z.number(),
  minutes: z.number(),
  seconds: z.number(),
  frames: z.number(),
...
const timecodeEventSchema = z.object({
  id: z.string(),
  label: z.string(),
  triggerTimecode: timecodeValueSchema,
  ...
  fired: z.boolean(),
  toleranceFrames: z.number().optional(),
});
...
export type MobileIncidentReactionEmoji = z.infer<
  typeof incidentReactionEmojiSchema
>;
...
export type MobileOrganizationInvitation = z.infer<
...
export type MobileDeviceControlState = z.infer<
  typeof mobileDeviceControlStateSchema
>;
...
export type MobileChecklistSuggestion = z.infer<
  typeof checklistSuggestionSchema
>;
...
export function parseMobileTimecodeState(
  value: unknown,
): MobileTimecodeState | null {
  const result = timecodeStateSchema.safeParse(value);
  return result.success ? result.data : null;
}

export function parseMobileTimecodeEvents(
  value: unknown,
): MobileTimecodeEvent[] | null {
  const result = z.array(timecodeEventSchema).safeParse(value);
  return result.success ? result.data : null;
}

export function parseMobileTimecodeEvent(
  value: unknown,
): MobileTimecodeEvent | null {
...
export function isRundownPinRequired(error: unknown): boolean {
  return (
...
async function authenticatedFetch(path: string, init?: FetchRequestInit) {
  ...
  const rundownOrgId = orgIdFromPinProtectedPath(path);
  const rundownPin = rundownOrgId
    ? await getStoredRundownPin(rundownOrgId)
    : null;
  ...
  if (!response.ok) {
    ...
    if (apiError) throw new MobileApiError(apiError, response.status, apiError);
    throw new MobileApiError(
      body || `ShowPilot request failed (${response.status})`,
      response.status,
      null,
    );
  }
...
export async function uploadMobileAvatar(uri: string) {
  ...
  if (Platform.OS === "web") {
    ...
    if (image.type !== "image/jpeg")
      throw new Error("The prepared profile photo is not a JPEG image.");
...
export async function getMobileBootstrap(
  ...
  const response = await authenticatedFetch(
    `/api/mobile/v1/bootstrap?orgId=${encodeURIComponent(orgId)}`,
  );
...
export async function getMobileNotificationPage(
  orgId: string,
  cursor: { createdAt: string; id: string },
) {
  const response = await authenticatedFetch(
    `/api/mobile/v1/notifications?orgId=${encodeURIComponent(orgId)}&beforeCreatedAt=${encodeURIComponent(cursor.createdAt)}&beforeId=${encodeURIComponent(cursor.id)}`,
  );
...
export async function getMobileChatMembers(orgId: string) {
  const response = await authenticatedFetch(
    `/api/mobile/v1/chat/members?orgId=${encodeURIComponent(orgId)}`,
  );
...
export async function getMobileContentSafety(orgId: string) {
  const response = await authenticatedFetch(
    `/api/mobile/v1/content-safety?orgId=${encodeURIComponent(orgId)}`,
  );
...
export async function reportMobileContent(input: {
  ...
  const response = await authenticatedFetch(
    `/api/mobile/v1/content-safety/report?orgId=${encodeURIComponent(input.orgId)}`,
    {
      method: "POST",
  ...
  return z
    .object({ ok: z.literal(true), reportId: z.string() })
    .parse(await response.json());
}

export async function blockMobileUser(input: {
  orgId: string;
  blockedUserId: string;
}) {
  const response = await authenticatedFetch(
    `/api/mobile/v1/content-safety/block?orgId=${encodeURIComponent(input.orgId)}`,
    {
      method: "POST",
      body: JSON.stringify(input),
    },
...
export async function downloadMobileChatAttachment(
  ...
  const safeName =
    attachment.name.replace(/[^a-zA-Z0-9._-]+/g, "-").slice(0, 120) ||
    "attachment";
...
export async function createMobileCrewChatPass(input: {
  orgId: string;
  hours: number;
}) {
  const response = await authenticatedFetch(
...
export async function createMobilePlanningChatPass(input: {
  orgId: string;
  hours: number;
...
export async function getMobileRundown(
  orgId: string,
  showId: string,
): Promise<MobileRundown> {
  const response = await authenticatedFetch(
...
export async function saveMobileRundownTemplate(input: {
  ...
  const response = await authenticatedFetch(
    ...
    {
      method: "POST",
      body: JSON.stringify({ requestId: input.requestId, name: input.name }),
    },
  );
  return z
    .object({ ok: z.literal(true), id: z.string(), created: z.boolean() })
    .parse(await response.json());
}
...
export async function updateMobileRundownMeta(input: {
  ...
  return z
    .object({ ok: z.literal(true), revision: z.number() })
    .parse(await response.json());
}
...
export async function controlMobileProPresenter(input: {
  ...
  return z
    .object({ success: z.literal(true), response: z.string().optional() })
    .parse(await response.json());
}
...
export async function updateMobileProPresenterStageDisplay(input: {
  ...
  return z
    .object({ ok: z.literal(true), enabled: z.boolean() })
    .parse(await response.json());
}
...
export async function createMobileScheduleAssignment(input: {
  ...
  const response = await authenticatedFetch(
    ...
    {
      method: "POST",
      body: JSON.stringify({ ...input, expectedUpdatedAt: null }),
    },
  );
...
export async function removeMobileScheduleAssignment(input: {
  orgId: string;
  assignmentId: string;
}) {
  const response = await authenticatedFetch(
...
export async function remindMobileScheduleAssignment(input: {
  orgId: string;
  assignmentId: string;
}) {
  const response = await authenticatedFetch(
...
export async function remindAllMobileScheduleAssignments(input: {
  orgId: string;
  showId: string;
}) {
  const response = await authenticatedFetch(
...
export async function removeMobileScheduleService(input: {
  orgId: string;
  showId: string;
}) {
  const response = await authenticatedFetch(
...
export async function getMobileIncidents(
  orgId: string,
): Promise<MobileIncidents> {
  const response = await authenticatedFetch(
    `/api/mobile/v1/incidents?orgId=${encodeURIComponent(orgId)}`,
  );
...
export async function getMobileIncidentHistory(
  orgId: string,
  filters: MobileIncidentHistoryFilters,
): Promise<MobileIncidentHistory> {
  ...
  const response = await authenticatedFetch(
    `/api/mobile/v1/incidents/history?${query}`,
  );
...
export async function reportMobileIncident(input: {
  ...
  const response = await authenticatedFetch(
    `/api/mobile/v1/incidents?orgId=${encodeURIComponent(input.orgId)}`,
    {
      method: "POST",
      body: JSON.stringify({ ...input, showId: input.showId ?? null }),
    },
  );
  return okSchema.parse(await response.json());
}
...
export async function removeMobileIncident(input: {
  orgId: string;
  incidentId: string;
}) {
  const response = await authenticatedFetch(
...
export async function addMobileIncidentComment(input: {
  ...
  return z
    .object({ comment: incidentCommentSchema })
    .parse(await response.json());
}
...
export async function setMobileIncidentCommentReaction(input: {
  ...
  const response = await authenticatedFetch(
    ...
    {
      method: "POST",
      body: JSON.stringify({ emoji: input.emoji, active: input.active }),
    },
  );
  return z
    .object({
...
export async function getMobileCheckIn(orgId: string): Promise<MobileCheckIn> {
  const response = await authenticatedFetch(
    `/api/mobile/v1/checkin?orgId=${encodeURIComponent(orgId)}`,
  );
  return checkInSchema.parse(await response.json());
}

export async function getMobileShowBoard(
  orgId: string,
): Promise<MobileShowBoard> {
  const response = await authenticatedFetch(
    `/api/mobile/v1/show-board?orgId=${encodeURIComponent(orgId)}`,
  );
  return showBoardSchema.parse(await response.json());
}

export async function getMobileShowWorkspace(
  orgId: string,
): Promise<MobileShowWorkspace> {
  const response = await authenticatedFetch(
    `/api/mobile/v1/show-workspace?orgId=${encodeURIComponent(orgId)}`,
  );
...
export async function getMobileTeamAccess(
  orgId: string,
): Promise<MobileTeamAccess> {
  const response = await authenticatedFetch(
    `/api/mobile/v1/team/access?orgId=${encodeURIComponent(orgId)}`,
  );
...
export async function grantMobileTeamAccess(input: {
  ...
  return z
    .object({ ok: z.literal(true), grantId: z.string() })
    .parse(await response.json());
}

export async function revokeMobileTeamAccess(input: {
  orgId: string;
  grantId: string;
}) {
  const response = await authenticatedFetch(
...
export async function getMobileTeamMembers(
  orgId: string,
): Promise<MobileTeamMembers> {
  const response = await authenticatedFetch(
    `/api/mobile/v1/team/members?orgId=${encodeURIComponent(orgId)}`,
  );
  return teamMembersSchema.parse(await response.json());
}

export async function inviteMobileTeamMember(input: {
  orgId: string;
  email: string;
  role: string;
}) {
  const response = await authenticatedFetch(
    `/api/mobile/v1/team/invitations?orgId=${encodeURIComponent(input.orgId)}`,
    {
      method: "POST",
...
export async function cancelMobileTeamInvitation(input: {
  orgId: string;
  invitationId: string;
}) {
  const response = await authenticatedFetch(
...
export async function updateMobileTeamMemberRole(input: {
  orgId: string;
  memberId: string;
  role: string;
}) {
...
export async function removeMobileTeamMember(input: {
  orgId: string;
  memberId: string;
}) {
  const response = await authenticatedFetch(
...
export async function getMobileTeamCrew(orgId: string) {
  const response = await authenticatedFetch(
    `/api/mobile/v1/team/crew?orgId=${encodeURIComponent(orgId)}`,
  );
...
export async function createMobileTeamCrewMember(input: MobileTeamCrewWrite) {
  const response = await authenticatedFetch(
    `/api/mobile/v1/team/crew?orgId=${encodeURIComponent(input.orgId)}`,
    {
      method: "POST",
      body: JSON.stringify(input),
    },
...
export async function updateMobileTeamCrewMember(
  input: MobileTeamCrewWrite & { id: string },
) {
  const response = await authenticatedFetch(
...
export async function removeMobileTeamCrewMember(input: {
  orgId: string;
  id: string;
}) {
  const response = await authenticatedFetch(
...
export async function getMobileChecklist(
  orgId: string,
  showId: string,
): Promise<MobileChecklist> {
  const query = new URLSearchParams({ orgId, showId });
  const response = await authenticatedFetch(
    `/api/mobile/v1/checklist?${query}`,
  );
  return mobileChecklistSchema.parse(await response.json());
}

export async function getMobileChecklistShows(orgId: string) {
  const response = await authenticatedFetch(
    `/api/mobile/v1/checklist/shows?orgId=${encodeURIComponent(orgId)}`,
  );
  return mobileChecklistShowsSchema.parse(await response.json()).shows;
}
...
export async function updateMobileChecklistCategory(input: {
  ...
  const response = await authenticatedFetch(
    `/api/mobile/v1/checklist/entries/${encodeURIComponent(input.entryId)}/category?orgId=${encodeURIComponent(input.orgId)}`,
    { method: "POST", body: JSON.stringify({ category: input.category }) },
  );
  return okSchema.parse(await response.json());
}

export async function getMobileChecklistDraft(
  orgId: string,
  showId: string,
): Promise<MobileChecklistDraft> {
  const query = new URLSearchParams({ orgId, showId });
  const response = await authenticatedFetch(
    `/api/mobile/v1/checklist/suggestions?${query}`,
  );
...
export async function getMobileDevices(orgId: string): Promise<MobileDevices> {
  const response = await authenticatedFetch(
    `/api/mobile/v1/devices?orgId=${encodeURIComponent(orgId)}`,
  );
...
export async function createMobileDevice(input: {
  ...
  const response = await authenticatedFetch(
    `/api/mobile/v1/devices?orgId=${encodeURIComponent(input.orgId)}`,
    {
      method: "POST",
      body: JSON.stringify(input),
    },
...
export async function removeMobileDevice(input: {
  orgId: string;
  deviceId: string;
}) {
  const response = await authenticatedFetch(
...
export async function getMobileDeviceControlState(input: {
  orgId: string;
  deviceId: string;
}) {
  const response = await authenticatedFetch(
...
export async function getMobileCueSheet(
  orgId: string,
  showId?: string,
): Promise<MobileCueSheet> {
  const query = new URLSearchParams({ orgId });
  if (showId) query.set("showId", showId);
  const response = await authenticatedFetch(
    `/api/mobile/v1/cue-sheets?${query}`,
  );
...
export async function writeMobileCueSheet(input: {
  ...
  const response = await authenticatedFetch(
    `/api/mobile/v1/cue-sheets?orgId=${encodeURIComponent(input.orgId)}`,
    {
      method: "POST",
      body: JSON.stringify(input),
    },
...
export async function getMobileAssets(orgId: string): Promise<MobileAssets> {
  const response = await authenticatedFetch(
    `/api/mobile/v1/assets?orgId=${encodeURIComponent(orgId)}`,
  );
...
export async function createMobileAsset(input: MobileAssetWrite) {
  const response = await authenticatedFetch(
    `/api/mobile/v1/assets?orgId=${encodeURIComponent(input.orgId)}`,
    {
      method: "POST",
      body: JSON.stringify(input),
    },
...
export async function updateMobileAsset(
  input: MobileAssetWrite & { id: string; expectedRevision: number },
) {
  const response = await authenticatedFetch(
    `/api/mobile/v1/assets/${encodeURIComponent(input.id)}?orgId=${encodeURIComponent(input.orgId)}`,
    {
      method: "POST",
      body: JSON.stringify(input),
    },
...
export async function removeMobileAsset(input: {
  orgId: string;
  id: string;
  expectedRevision: number;
}) {
...
export async function getMobileStreaming(
  orgId: string,
): Promise<MobileStreaming> {
  const response = await authenticatedFetch(
    `/api/mobile/v1/streaming?orgId=${encodeURIComponent(orgId)}`,
  );
...
export async function createMobileDestination(input: MobileDestinationWrite) {
  const response = await authenticatedFetch(
    `/api/mobile/v1/streaming/destinations?orgId=${encodeURIComponent(input.orgId)}`,
    {
      method: "POST",
      body: JSON.stringify(input),
    },
...
export async function updateMobileDestination(
  input: MobileDestinationWrite & { id: string },
) {
  const response = await authenticatedFetch(
    `/api/mobile/v1/streaming/destinations/${encodeURIComponent(input.id)}?orgId=${encodeURIComponent(input.orgId)}`,
    {
      method: "POST",
      body: JSON.stringify(input),
    },
...
export async function commandMobileDestination(input: {
  orgId: string;
  id: string;
...
export async function getMobileGraphics(
  orgId: string,
): Promise<MobileGraphics> {
  const response = await authenticatedFetch(
    `/api/mobile/v1/graphics?orgId=${encodeURIComponent(orgId)}`,
  );
...
export async function createMobileGraphic(input: MobileGraphicWrite) {
  const response = await authenticatedFetch(
    `/api/mobile/v1/graphics?orgId=${encodeURIComponent(input.orgId)}`,
    {
      method: "POST",
      body: JSON.stringify(input),
    },
...
export async function updateMobileGraphic(
  input: MobileGraphicWrite & { id: string },
) {
  const response = await authenticatedFetch(
    `/api/mobile/v1/graphics/${encodeURIComponent(input.id)}?orgId=${encodeURIComponent(input.orgId)}`,
    {
      method: "POST",
      body: JSON.stringify(input),
    },
...
export async function commandMobileGraphic(input: {
  orgId: string;
  id?: string;
  action: "toggle" | "clear" | "remove";
}) {
  const path = input.id
    ? `/api/mobile/v1/graphics/${encodeURIComponent(input.id)}`
    : "/api/mobile/v1/graphics";
  const response = await authenticatedFetch(
    `${path}?orgId=${encodeURIComponent(input.orgId)}`,
    {
      method: "POST",
      body: JSON.stringify(input),
    },
  ...
  return okSchema
    .extend({ activeIds: z.array(z.string()).optional() })
    .parse(await response.json());
}
...
export async function getMobileDashboard(
  ...
  if (showId) query.set("showId", showId);
  const response = await authenticatedFetch(
    `/api/mobile/v1/dashboards/${kind}?${query}`,
  );
...
export async function getMobileReports(orgId: string): Promise<MobileReports> {
  const response = await authenticatedFetch(
    `/api/mobile/v1/reports?orgId=${encodeURIComponent(orgId)}`,
  );
...
export async function saveMobileShowReportNote(input: {
  ...
  const response = await authenticatedFetch(
    `/api/mobile/v1/reports/${encodeURIComponent(input.showId)}/notes?orgId=${encodeURIComponent(input.orgId)}`,
    {
      method: "POST",
      body: JSON.stringify(input),
    },
...
export async function getMobileAudio(
  orgId: string,
  showId?: string,
): Promise<MobileAudio> {
  const query = new URLSearchParams({ orgId });
...
export async function createMobileAudioAssignment(input: MobileAudioWrite) {
  const response = await authenticatedFetch(
    `/api/mobile/v1/audio?orgId=${encodeURIComponent(input.orgId)}`,
    {
      method: "POST",
      body: JSON.stringify(input),
    },
...
export async function updateMobileAudioAssignment(
  input: MobileAudioWrite & { id: string },
) {
  const response = await authenticatedFetch(
    `/api/mobile/v1/audio/${encodeURIComponent(input.id)}?orgId=${encodeURIComponent(input.orgId)}`,
    {
      method: "POST",
      body: JSON.stringify(input),
    },
...
export async function removeMobileAudioAssignment(input: {
  orgId: string;
  id: string;
}) {
  const response = await authenticatedFetch(
...
export async function getMobileTimecode(
  orgId: string,
): Promise<{ state: MobileTimecodeState; events: MobileTimecodeEvent[] }> {
...
export async function commandMobileTimecode(input: {
  ...
  const response = await authenticatedFetch(
    `/api/timecode/${encodeURIComponent(input.orgId)}/command`,
    {
      method: "POST",

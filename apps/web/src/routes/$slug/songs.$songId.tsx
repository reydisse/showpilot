...
function SongStudioPage() {
  ...
  const removeSong = async () => {
    ...
    try {
      ...
      setDeleteError(cause instanceof Error ? cause.message : "Song could not be deleted.");
    } finally {
      setDeleteBusy(false);
    }
  };
  ...
  return <div className="min-h-full bg-board-bg p-4 sm:p-6"><div className="mx-auto max-w-7xl space-y-5">
    <header className="flex flex-wrap items-center justify-between gap-3">
      <div className="flex min-w-0 items-center gap-3">
        <Link to="/$slug/songs" params={{ slug }} aria-label="Back to songs" className="rounded-xl border border-board-border bg-board-card p-2.5 text-board-muted"><ArrowLeft className="h-4 w-4" /></Link>
        <div className="min-w-0"><p className="text-xs font-semibold uppercase tracking-[0.16em] text-fire-500">Song studio</p><h1 className="truncate text-xl font-semibold text-board-text">{metadata.title}</h1></div>
      </div>
      <div className="flex flex-wrap items-center justify-end gap-2">
        {song.ppPresentationUuid && canManage ? <button disabled={busy} onClick={() => void reimport()} className="inline-flex min-h-10 items-center gap-1.5 rounded-lg border border-board-border bg-board-card px-3 text-xs font-semibold text-board-text disabled:opacity-40"><RefreshCw className="h-3.5 w-3.5" />Check ProPresenter</button> : null}
        {canManage ? <button type="button" onClick={() => { setDeleteError(null); setDeleteOpen(true); }} className="inline-flex min-h-10 items-center gap-1.5 rounded-lg border border-red-500/30 bg-red-500/5 px-3 text-xs font-semibold text-red-300 hover:bg-red-500/10"><Trash2 className="h-3.5 w-3.5" />Delete song</button> : null}
        <div className="flex items-center gap-2 text-xs text-board-muted"><span className={`h-2 w-2 rounded-full ${timecode.connected ? "bg-green-400" : "bg-red-400"}`} />{timecode.connected ? timecode.display : "Timecode offline"}</div>
      </div>
    </header>
      ...
      <main className="space-y-5">
        ...
        <section className="flex min-h-80 flex-col rounded-2xl border border-board-border bg-board-card p-5 xl:max-h-[calc(100dvh-22rem)]">
          <div className="flex shrink-0 items-center justify-between gap-3">
            <div><h2 className="font-semibold text-board-text">Arrangement</h2><p className="mt-1 text-xs text-board-muted">Drag sections into show order.</p></div>
            {canManage ? <button type="button" onClick={async () => { await saveSongSection({ data: { orgId, songId: song.id, label: `Section ${sections.length + 1}`, lyrics: "", sourceSlideIndex: null } }); await refresh(); }} className="inline-flex shrink-0 items-center gap-2 rounded-lg border border-board-border px-3 py-2 text-xs font-semibold text-board-text"><Plus className="h-3.5 w-3.5" />Section</button> : null}
          </div>
          <div className="modern-scrollbar mt-4 min-h-0 space-y-3 xl:flex-1 xl:overflow-y-auto xl:overscroll-contain xl:pr-2">
            {sections.map((section, index) => <div key={section.id} draggable={canManage} onDragStart={() => setDraggedSectionId(section.id)} onDragEnd={() => setDraggedSectionId(null)} onDragOver={(event) => event.preventDefault()} onDrop={() => { const from = sections.findIndex((item) => item.id === draggedSectionId); setDraggedSectionId(null); void moveSection(from, index); }} className={`rounded-xl border bg-board-bg p-3 ${draggedSectionId === section.id ? "border-fire-500/60 opacity-60" : "border-board-border"}`}><div className="flex gap-2">{canManage ? <span aria-hidden="true" className="cursor-grab p-2 text-board-muted"><GripVertical className="h-4 w-4" /></span> : null}<input value={section.label} onChange={(event) => setSections((current) => current.map((item) => item.id === section.id ? { ...item, label: event.target.value } : item))} className="min-w-0 flex-1 bg-transparent text-sm font-semibold text-board-text outline-none" /><button aria-label="Move section up" disabled={index === 0} onClick={() => void moveSection(index, index - 1)} className="p-2 text-board-muted disabled:opacity-20"><ChevronUp className="h-4 w-4" /></button><button aria-label="Move section down" disabled={index === sections.length - 1} onClick={() => void moveSection(index, index + 1)} className="p-2 text-board-muted disabled:opacity-20"><ChevronDown className="h-4 w-4" /></button></div><textarea value={section.lyrics} onChange={(event) => setSections((current) => current.map((item) => item.id === section.id ? { ...item, lyrics: event.target.value } : item))} onPaste={(event) => { event.preventDefault(); const lyrics = insertPastedLyrics({ value: section.lyrics, pastedText: event.clipboardData.getData("text/plain"), selectionStart: event.currentTarget.selectionStart, selectionEnd: event.currentTarget.selectionEnd }); setSections((current) => current.map((item) => item.id === section.id ? { ...item, lyrics } : item)); }} rows={4} placeholder="Lyrics, up to four display lines per section" className="mt-2 w-full resize-y rounded-lg border border-board-border bg-board-card p-3 text-sm leading-6 text-board-text outline-none" />{canManage ? <div className="mt-2 flex justify-between"><button onClick={() => timecode.setLyrics({ songId: song.id, songTitle: metadata.title, sectionId: section.id, sectionLabel: section.label, lyrics: section.lyrics, nextLabel: sections[index + 1]?.label ?? "" })} className="inline-flex items-center gap-1.5 text-xs font-medium text-blue-300"><MonitorPlay className="h-3.5 w-3.5" />Show now</button><div className="flex gap-2"><button onClick={() => void saveSongSection({ data: { orgId, songId: song.id, sectionId: section.id, label: section.label, lyrics: section.lyrics, sourceSlideIndex: section.sourceSlideIndex } }).then(refresh)} className="text-xs font-semibold text-fire-500">Save section</button><button aria-label={`Delete ${section.label}`} onClick={() => { const mapped = song.cueMaps.some((map) => map.cues.some((cue) => cue.sectionId === section.id)); if (mapped && !window.confirm(`Delete ${section.label}? This will also remove its mapped cues.`)) return; void deleteSongSection({ data: { orgId, songId: song.id, sectionId: section.id, confirmMapped: mapped } }).then(refresh).catch((cause: unknown) => setNotice(cause instanceof Error ? cause.message : "Section did not delete.")); }} className="text-red-400"><Trash2 className="h-3.5 w-3.5" /></button></div></div> : null}</div>)}
          </div>
        </section>
      </main>
    ...
    {deleteOpen ? <DeleteSongModal title={metadata.title} sectionCount={song.sections.length} cueMapCount={song.cueMaps.length} busy={deleteBusy} error={deleteError} onClose={() => { if (!deleteBusy) setDeleteOpen(false); }} onDelete={() => void removeSong()} /> : null}
  </div></div>;
}

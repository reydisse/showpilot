...
function TimecodePage() {
  ...
  return (
    <div className="h-full overflow-auto">
      ...
      {songLoaderOpen ? <SongCueLoader orgId={orgId} defaultTarget={lyricsTarget(settings["lyrics-output"])} format={state?.format ?? { frameRate: 30, dropFrame: "ndf" }} onClose={() => setSongLoaderOpen(false)} onLoad={(loaded) => { const sourceKey = loaded[0]?.sourceKey; if (sourceKey) replaceEventGroup(sourceKey, loaded); setSongLoaderOpen(false); }} /> : null}
    </div>
  );
}

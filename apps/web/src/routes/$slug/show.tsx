...
import { getOntimeState, formatOntimeTime, formatDuration } from "@/lib/ontime";
...
export const Route = createFileRoute("/$slug/show")({
  loader: async ({ context }) => {
    const [members, ontimeState, chatMessages] = await Promise.all([
      getCrewMembers({ data: { orgId: context.orgId } }),
      getOntimeState({ data: { orgId: context.orgId } }),
      getChatMessages({ data: { orgId: context.orgId, limit: 50 } }),
    ]);
    return {
      members,
      initialOntime: ontimeState,
  ...
  useEffect(() => {
    const poll = async () => {
      try {
        ...
        setError("Cannot connect to chat");
      }
    };
  ...
  const handleSend = async () => {
    ...
    try {
      await sendChatMessage({
        data: { orgId, message: input.trim(), senderName: senderName.trim() },
      });
      setInput("");
      const fresh = await getChatMessages({ data: { orgId, limit: 50 } });
      setMessages(fresh);
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex items-center gap-2 pb-3 border-b border-board-border/50">
        ...
        {senderName && !showNameInput && (
          <button
            ...
            className="ml-auto text-xs text-board-muted hover:text-board-text transition-colors"
          >
            {senderName}
          </button>
        )}
      ...
      {showNameInput && (
        <div className="py-3 border-b border-board-border/50">
          <label className="text-xs text-board-muted mb-1.5 block">
            Your name
          </label>
          <div className="flex gap-2">
            ...
            <button
              ...
              className="px-3 py-1.5 rounded-lg bg-fire-500 text-white text-xs font-medium disabled:opacity-40 transition-colors"
            >
              Set
            </button>
          </div>
      ...
      <div
        ref={scrollRef}
        className="flex-1 overflow-auto py-3 space-y-3 hide-scrollbar min-h-0"
      >
        {error ? (
          ...
          <p className="text-center text-board-muted/50 text-xs py-4">
            No messages yet
          </p>
        ) : (
          sortedMessages.map((msg) => (
            <div key={msg.id} className="group">
              <div className="flex items-baseline gap-2">
                <span className="text-xs font-medium text-fire-500 shrink-0">
                  {msg.senderName}
                </span>
              ...
              <p className="text-sm text-board-text/90 mt-0.5 leading-relaxed break-words">
                {msg.message}
              </p>
            </div>
          ))
...
function ShowPage() {
  const { members, initialOntime, initialChat, orgId, slug } =
    Route.useLoaderData();
  ...
  const isOvertime =
    ontime.timer.current !== null && ontime.timer.current < 0;

  // Poll OnTime state
  useEffect(() => {
    const poll = async () => {
      try {
        const state = await getOntimeState({ data: { orgId } });
        setOntime(state);
      } catch {
  ...
  // Detect transition to "play" and show the live flash
  const [showFlash, setShowFlash] = useState(false);
  const prevPlayback = useRef(ontime.timer.playback);

  useEffect(() => {
    if (
      ontime.timer.playback === "play" &&
      prevPlayback.current !== "play"
    ) {
      setShowFlash(true);
    }
    ...
    <>
      ...
      <div className="h-full flex flex-col overflow-hidden">
        ...
        <main className="flex-1 overflow-hidden px-8 py-5">
          {!ontime.connected && ontime.events.length === 0 ? (
            <div className="flex gap-6 h-full max-w-[1400px] mx-auto">
              {/* Left column: Offline timer placeholder + Chat */}
              <div className="w-[400px] shrink-0 flex flex-col gap-5 h-full">
                <div className="p-6 rounded-xl border bg-board-card border-board-border shrink-0">
                  ...
                  <p className="text-4xl font-semibold tabular-nums text-board-muted">
                    --:--
                  </p>
                  <p className="text-xs text-board-muted/60 mt-3">
                    Configure an OnTime device in{" "}
                    <Link
                ...
                >
                  ...
                  <p
                    className={`text-6xl font-semibold tabular-nums tracking-tight ${
                      isOvertime ? "text-red-400" : "text-board-text"
                    }`}
                  >
                    {formatDuration(ontime.timer.current)}
                  </p>
                  ...
                  {ontime.eventNext && (
                    <p className="text-xs text-board-muted mt-2">
                      Next:{" "}
                      <span className="text-board-text/70">
                        {ontime.eventNext.title}
                      </span>
                    </p>
                  )}
                </div>
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            {event.note && (
                              <p className="text-xs text-board-muted/60 truncate mt-0.5">
                                {event.note}
                              </p>
                            )}
                          </div>
        ...
        {activeMembers.length > 0 && (
          <footer className="shrink-0 border-t border-board-border py-2.5 overflow-hidden">
            <div className="flex items-center gap-3 px-8">
              <span className="text-[10px] font-medium text-board-muted uppercase tracking-widest shrink-0">
                On Crew
              </span>
